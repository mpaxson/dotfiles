...
| **Dynamic variables** | `{{ USER_NAME }}`, `{{ CURRENT_DATE }}`, `{{ CURRENT_TIME }}` injected automatically |
...
| :--- | :--- |
| **Knowledge** | Bind collections or files. Click attached items to toggle between Focused Retrieval and Full Context |
...
Administrators can set baseline capabilities and parameters that apply to all models via **Admin Panel > Settings > Models > gear icon**.
