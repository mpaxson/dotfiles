...
|------|:-----------:|:--------------:|-------------|
| `list_knowledge` | Yes | No | List all KBs, files, and notes attached to the model |
| `list_knowledge_bases` | No | Yes | Browse available knowledge bases with file counts |
| `search_knowledge_bases` | No | Yes | Find knowledge bases by name or description |
| `query_knowledge_bases` | No | Yes | Search KB names/descriptions by semantic similarity |
| `search_knowledge_files` | Yes (scoped) | Yes (all) | Search files by filename |
| `query_knowledge_files` | Yes (scoped) | Yes | Search file contents using the RAG pipeline |
| `view_file` | Yes | No | Read file content with pagination (default 10K chars, cap 100K) |
| `view_knowledge_file` | Yes | Yes | Read file content from any accessible KB |
| `view_note` | Yes | No | Read attached notes |
...
**Important**: When native function calling is enabled, attached knowledge is **not automatically injected**. The model must call the knowledge tools to search and retrieve. If your model isn't using attached knowledge:
...
File processing happens asynchronously. You must poll the status endpoint until processing completes before adding files to a knowledge base, or you'll get an "empty content" error.
...
Enabling native function calling changes how knowledge bases work. If your KB suddenly stops producing results, check whether `function_calling: native` was set in global model defaults.
