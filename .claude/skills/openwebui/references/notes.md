...
Notes is a dedicated workspace for content that lives outside of any single conversation: drafts, cheat sheets, research findings, project briefs. Things you build up over time, refine with AI, and inject into future chats as context.
...
With native function calling enabled, models can search, read, create, and update your notes autonomously, turning Notes into a dynamic long-term memory.

## Key Features

| Feature | Description |
| :--- | :--- |
| **Rich editor** | Markdown and Rich Text with a floating formatting toolbar |
...
With **native function calling** enabled, models can work with your notes autonomously:
...
- "Search my 'Project X' notes and find the database schema."
- "Add a new task to my 'Weekly Todo' note to review the PR."
...
Notes are workspace items. By default (`BYPASS_ADMIN_ACCESS_CONTROL=True`), administrators can see all users' notes. Set `BYPASS_ADMIN_ACCESS_CONTROL` to `False` to restrict this.
