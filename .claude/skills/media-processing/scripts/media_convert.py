...
def check_dependencies() -> Tuple[bool, bool]:
    ...
    magick_available = (
        subprocess.run(
            ["magick", "-version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
        ).returncode
        == 0
    )
...
def build_video_command(
    input_path: Path, output_path: Path, preset: str = "web"
) -> List[str]:
    ...
    for input_path in input_paths:
        ...
        if output_dir:
            ...
            if output_format:
                ...
                print(
                    f"Error: No output format specified for {input_path}",
                    file=sys.stderr,
                )
                fail_count += 1
                continue
...
def main():
    ...
    parser.add_argument("inputs", nargs="+", type=Path, help="Input file(s) to convert")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
    ...
    parser.add_argument("-f", "--format", help="Output format (e.g., mp4, jpg, mp3)")
    parser.add_argument(
        "-p",
        "--preset",
    ...
    if len(args.inputs) == 1 and args.output and not args.output.is_dir():
        ...
        success = convert_file(
            args.inputs[0], args.output, args.preset, args.dry_run, args.verbose
        )
