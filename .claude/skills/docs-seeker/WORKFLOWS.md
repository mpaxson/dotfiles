...
Step 2: Search for llms.txt
→ WebSearch: "shadcn ui llms.txt"
→ Result: https://ui.shadcn.com/llms.txt found
→ WebFetch: Read llms.txt content

Step 3: Filter for relevant URLs
→ Identify date-related documentation URLs from llms.txt
→ Launch Explorer agent for date picker docs only
